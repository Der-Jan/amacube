...
class AmavisSettings extends AmavisAbstract
{
    ...
    */
    ...
    function write_to_db()
    {
        ...
        // error check
        if($this->db_conn->is_error($res)) {
            return "Error in insert/update policy: ".$this->db_conn->db_error();
        }
        ...
        if(empty($this->policy_pk)) {
            ...
            // error check
            if(empty($this->policy_pk)) {
                return "Could not get Primary Key for policy: ".$this->db_conn->db_error();
            }
            ...
            // error check
            if($this->db_conn->is_error($res)) {
                return "Error in checking for user record: ".$this->db_conn->db_error();
            }
            ...
            //  error check
            if($this->db_conn->is_error($res2)) {
                return "Error in inserting/updating user record: ".$this->db_conn->db_error();
            }
        }
        // all good:
        return null;
